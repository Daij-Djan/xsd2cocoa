...
{
	NSRange matchRange = [self.templateString rangeOfRegex:self.regex options:0 inRange:range capture:0 error:NULL];
	...
	if (matchRange.length > 0) {
		...
		NSRange mrkrSubRange = [matchString rangeOfRegex:regex options:0 inRange:localRange capture:1 error:NULL];
		...
		markerRange = [matchString rangeOfRegex:regex options:0 inRange:localRange capture:2 + offset error:NULL];
		...
		if (markerRange.length > 0) {
			...
			// Check for filter.
			NSRange filterRange = [matchString rangeOfRegex:regex options:0 inRange:localRange capture:3 + offset error:NULL];
			if (filterRange.length > 0) {
				...
				NSString *space = @" ";
				NSRange filterArgDelimRange = [filterString rangeOfRegex:@":(?:\\s+)?" options:0 inRange:localRange
...
- (NSArray *)argumentsFromString:(NSString *)argString
{
	...
	while (location != NSNotFound) {
		...
		NSRange entireRange = [argString rangeOfRegex:argsPattern options:0
											  inRange:searchRange capture:0 error:NULL];
		NSRange matchedRange = [argString rangeOfRegex:argsPattern options:0
											   inRange:searchRange capture:1 error:NULL];
		if (matchedRange.length == 0) {
			matchedRange = [argString rangeOfRegex:argsPattern options:0
